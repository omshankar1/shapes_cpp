// @file sfml_grid.cpp
// g++ -std=c++17 sfml_grid.cpp -o prog -lpthread -lsfml-graphics -lsfml-window
// -lsfml-system
...
int main() {
  ...
  for (int x = 0; x < 2; x++) {
    for (int y = 0; y < 2; y++) {
      shapes.push_back(
          std::make_unique<sf::RectangleShape>(sf::Vector2f(100, 100)));
      // std::make_unique<sf::CircleShape>(100.0f));
    }
  }
